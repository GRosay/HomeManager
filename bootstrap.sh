...
apt-get -y install mysql-server phpmyadmin php-mbstring php-gettext >> /vagrant/vm_build.log 2>&1
phpenmod mcrypt >> /vagrant/vm_build.log 2>&1
phpenmod mbstring >> /vagrant/vm_build.log 2>&1
...
echo -e "\n--- IMPORT INITIAL DATABASE DATAS ---\n"
mysql -uroot -p$DBPASSWD $DBNAME < /vagrant/database.sql >> /vagrant/vm_build.log 2>&1
